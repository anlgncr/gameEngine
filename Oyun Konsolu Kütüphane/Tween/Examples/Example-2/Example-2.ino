...
void move(EventArgs* e){
  ...
  if(manager.isButtonDown(BUTTON_B)){
    Tween::tweenObject *a = Tween::start(d->getXRef(), d->getX(), 103, 250, SPIMEM);
    Tween::setAnimation(a, EASE_IN);
  }
  else if(manager.isButtonDown(BUTTON_D)){
    Tween::tweenObject *a = Tween::start(d->getXRef(), d->getX(), 0, 250, SPIMEM);
    Tween::setAnimation(a, EASE_OUT);
  }
}
